...
export default function App() {
  ...
  const [selectedLectureForVideo, setSelectedLectureForVideo] =
    useState<Lecture | null>(null);
  const [isVideoOpen, setIsVideoOpen] = useState(false);
  const [isTeacherActive, setIsTeacherActive] = useState(false);

  // Blog detail states
  const [selectedBlogForDetail, setSelectedBlogForDetail] =
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      lecturesQuery,
      (snapshot) => {
        const serverLectures: Lecture[] = snapshot.docs.map((lectureDoc) => {
          const data = lectureDoc.data() as any;
          return {
            id: lectureDoc.id,
            title: data.title || "",
            ...
            timeAgo: data.timeAgo || "Vừa xong",
            videoUrl: data.videoUrl || undefined,
          };
        });

        setAllLectures(serverLectures);
        localStorage.setItem(
          "thaynguyentoan_lectures",
          JSON.stringify(serverLectures),
        );
      },
      (error) => {
        console.error("Firestore lectures snapshot error:", error);
      },
    );
  ...
  const handleSubmitRegistration = (registration: {
    name: string;
    phone: string;
    email: string;
    grade: string;
  ...
  const handleNavClick = (sectionId: string) => {
    ...
    if (sectionId === "documents") {
      ...
      setSection("home");
      setTimeout(() => {
        const teachingSection = document.getElementById(
  ...
  const handleAddLecture = async (
    newLecture: Omit<Lecture, "id" | "views" | "timeAgo">,
  ) => {
    const randomizedViews = Math.floor(Math.random() * 5 + 1) + ".1K lượt xem";
    const lectureId = `lec-added-${Date.now()}`;
    ...
    try {
      await setDoc(doc(collection(db, "lectures"), lectureId), {
        ...lectureToAppend,
        createdAt: serverTimestamp(),
      });
    } catch (error) {
      console.error("Cannot persist lecture to Firestore:", error);
    }
  };
  ...
  const handleDeleteLecture = async (id: string) => {
    ...
    try {
      ...
      console.warn("Cannot delete lecture from Firestore:", error);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#060913] text-white flex flex-col font-sans">
      ...
      <Header
        ...
        onOpenTeacherDashboard={() => {
          if (currentUser?.email === "triet5509@gmail.com") {
            ...
            alert(
              "🔒 PHÂN QUYỀN QUẢN TRỊ: Chức năng này chỉ cấp phép cho tài khoản quản trị triet5509@gmail.com!",
            );
          }
        }}
      />
          ...
          <Hero
            ...
            onPlayIntro={() => {
              handleSelectLecture({
                id: "lec-intro",
                title:
                  "Chào mừng bạn đến với Nền Tảng Học Toán Trực Tuyến Thầy Nguyên",
                category: "Giới thiệu",
                views: "10K lượt xem",
                timeAgo: "Hôm nay",
                duration: "02:30",
          ...
          <MoMoPayment
            onInitiatePayment={() => {
              if (!currentUser) {
                ...
                setIsAuthOpen(true);
                alert(
                  "Vui lòng đăng nhập trước khi kích hoạt thanh toán học tập nhé!",
                );
                return;
              }
      ...
      <VideoModal
        ...
        title={
          selectedLectureForVideo?.title ||
          "Đột phá điểm 9+: Chinh phục đồ thị hàm số tuyệt đối"
        }
