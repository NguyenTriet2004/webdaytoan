...
export default function TeachingMethods({
  ...
  currentUser,
  documents = [],
}: TeachingMethodsProps) {
  const [activeTab, setActiveTab] = useState<"video" | "documents">("video");

  return (
    <section
      id="teaching-methods-section"
      className="py-20 bg-[#060913] text-white overflow-hidden relative border-t border-b border-slate-800"
    >
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full bg-emerald-600/5 blur-[150px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-4">
          ...
          <p className="text-slate-400 text-sm max-w-2xl mx-auto font-sans leading-relaxed">
            Thầy Nguyên mang đến mô hình đào tạo kết hợp bản quyền: Tự học tiện
            lợi qua kho chuyên đề bài giảng số đỉnh cao và bứt phá thực hành
            cùng thư viện hệ thống giáo trình, tài liệu tự luyện phong phú.
          </p>
              ...
              >
                ...
                <div className="lg:col-span-5 space-y-6 text-left">
                  <div className="space-y-2">
                    <span className="text-xs uppercase font-extrabold text-emerald-400 font-mono tracking-widest">
                  ...
                  <p className="text-xs sm:text-sm text-slate-400 leading-relaxed font-sans">
                    Học viên được cấp quyền truy cập trọn đời nền tảng bài giảng
                    video đã quay dựng sẵn với chất lượng 1080P sắc nét. Mỗi
                    chuyên đề bao trùm đầy đủ từ lý thuyết cốt lõi đến sơ đồ
                    phản xạ tư duy nhanh và các phương thức Casio bứt tốc kiểm
                    tra trắc nghiệm.
                  </p>
                  ...
                  <div className="space-y-3.5 pt-2">
                    {[
                      ...
                      <div
                        key={idx}
                        className="flex items-start space-x-2.5 text-xs text-slate-300 font-medium"
                      >
                        <span className="w-5 h-5 rounded-full bg-emerald-500/10 border border-emerald-500/25 flex items-center justify-center text-emerald-400 text-[10px] font-bold font-mono mt-0.5">
                          {idx + 1}
                        </span>
                ...
                <div className="lg:col-span-7 bg-slate-900/40 border border-slate-800/80 rounded-2xl p-4 sm:p-5 text-left">
                  <div className="relative aspect-video rounded-xl bg-slate-950 overflow-hidden group shadow-2xl border border-slate-850">
                      ...
                      <button
                        onClick={() =>
                          onSelectLecture({
                            id: "preview-video",
                            title:
                              "Đột phá điểm 9+: Chinh phục đồ thị hàm số chứa dấu giá trị tuyệt đối cực nâng cao",
                            category: "Khóa nâng cao",
                            videoUrl:
                              "https://www.youtube.com/watch?v=j2D-lgB0IOI",
                          })
                        }
                        className="relative z-25 p-4 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white shadow-xl shadow-emerald-500/30 group-hover:scale-110 active:scale-95 transition-all duration-300 cursor-pointer"
                      >
                    ...
                    <div className="mt-4 flex items-center justify-between text-xs text-slate-450 font-mono">
                      <span>Thời lượng phát: 16 phút 50 giây</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex items-center justify-between text-xs text-slate-450 font-mono">
                    ...
                    <button
                      ...
                      className="text-emerald-400 font-bold hover:underline cursor-pointer bg-transparent border-none outline-none"
                    >
              ...
              >
                {documents.length === 0 ? (
                  <div className="col-span-full py-16 px-6 bg-slate-900/40 border border-slate-800 rounded-3xl text-center space-y-4 max-w-lg mx-auto">
                    ...
                    <div className="space-y-1">
                      <h3 className="text-base font-extrabold text-slate-200">
                        Kho tài liệu đang cập nhật
                      </h3>
                      <p className="text-xs text-slate-400 font-sans leading-relaxed">
                        Tài liệu phác đồ và tài liệu kiểm tra sẽ được Thầy
                        Nguyên cập nhật trực tiếp tại đây. Đăng ký nhận khóa học
                        để mở khóa toàn bộ kho tài liệu chuyên sâu!
                      </p>
                    </div>
                  </div>
                ) : (
                  documents.map((doc) => (
                    <div
                      key={doc.id}
                      className="p-5 bg-slate-900/60 border border-slate-800/80 hover:border-emerald-500/50 rounded-2xl flex flex-col justify-between space-y-4 hover:shadow-xl hover:shadow-emerald-900/5 transition-all group text-left"
                    >
                      <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between border-t border-slate-800/50 pt-3 flex-wrap gap-2 text-xs">
                        ...
                        <button
                          ...
                          className="flex items-center space-x-1.5 px-3 py-1.5 bg-emerald-600/10 hover:bg-emerald-500 border border-emerald-500/20 hover:border-emerald-600 text-emerald-400 hover:text-white font-bold rounded-xl transition-all text-[11px] cursor-pointer"
                        >
